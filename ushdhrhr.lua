-- DOORS ULTRA Premium Menu + Mobile Support (FULL FIX)
...
-- ===== БОЛЬШАЯ МОБИЛЬНАЯ КНОПКА ОТКРЫТИЯ =====
local mobileButton = Instance.new("TextButton")
mobileButton.Size = UDim2.new(0, 70, 0, 70) -- Большая кнопка
mobileButton.Position = UDim2.new(0.04, 0, 0.88, 0) -- Левый нижний угол
...
mobileButton.Visible = isMobile -- Показываем только на телефонах
...
-- Текст "MENU" под кнопкой (на телефоне)
local menuLabel = Instance.new("TextLabel")
menuLabel.Size = UDim2.new(0, 70, 0, 20)
menuLabel.Position = UDim2.new(0.04, 0, 0.88, 70)
...
local function createModule(parent, yPos, labelText, icon, getter, setter, settingsCallback)
    ...
    -- Кнопка с тремя полосками (настройки модуля)
...
createModule(mainFrame, 120, "ESP Book", "📖",
    ...
    function(parent, open)
        if open then
            ...
            UserInputService.InputChanged:Connect(function(input)
                if dragging and (input.UserInputType == Enum.UserInputType.MouseMov
