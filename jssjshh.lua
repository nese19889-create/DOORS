...
-- 1. Яркость (ФИКС - сохраняем настройки в Lighting)
...
-- === ФИКС: СОХРАНЯЕМ ЯРКОСТЬ ПРИ КАЖДОМ ИЗМЕНЕНИИ КАМЕРЫ ===
-- Игра в Doors часто меняет настройки освещения при переключении комнат
-- Поэтому мы будем постоянно проверять и восстанавливать яркость
game:GetService("RunService").Heartbeat:Connect(function()
    if brightOn then
        local lighting = game:GetService("Lighting")
        -- Если яркость сбилась - восстанавливаем
...
print("✅ Мод-меню загружен! Яркость теперь не сбрасывается")
