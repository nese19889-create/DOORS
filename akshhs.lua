-- DOORS ULTRA Premium Menu + Fix Head + Module Settings
...
-- ===== ФУНКЦИЯ СОЗДАНИЯ МОДУЛЯ С ТРЕМЯ ПОЛОСКАМИ =====
local function createModule(parent, yPos, labelText, icon, getter, setter, settingsCallback)
    ...
    -- Контейнер для настроек модуля (сворачиваемый)
    ...
    -- Кнопка с тремя полосками (гамбургер)
    ...
    menuBtn.MouseButton1Click:Connect(function()
        ...
        -- Анимация полосок (превращаются в крестик)
        local stripes = {}
        ...
        if settingsOpen then
            -- Поворачиваем полоски в крестик
...
-- ===== СОЗДАНИЕ НАСТРОЕК ДЛЯ FULLBRIGHT =====
...
-- ===== СОЗДАНИЕ МОДУЛЕЙ =====
-- Fullbright модуль
local fullbrightModule, fullbrightSettings = createModule(mainFrame, 65, "Fullbright", "💡", 
...
-- ESP модуль
local espModule, espSettings = createModule(mainFrame, 120, "ESP Book", "📖",
...
-- ===== СЛАЙДЕР JUMP POWER (оставляем отдельно) =====
local function createSlider(parent, yPos)
    ...
    label.Position = UDim2.new(0, 10, 0, 0)
    label.BackgroundTransparency = 1
    label.Text = "⚡ Jump Power: " .. settings.JumpPower
    label.TextColor3 = Color3.fromRGB(230, 230, 250)
    label.TextXAlignment = Enum.TextXAlignment.Left
    label.TextScaled = true
    label.Font = Enum.Font.GothamMedium
    ...
    valueLabel.Text = settings.JumpPower
    ...
    sliderBg.Position = UDim2.new(0.07, 0, 0.7, 0)
    sliderBg.BackgroundColor3 = Color3.
